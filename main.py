...
import calendar
import re
from sqlalchemy import create_engine # Necesario para la base de datos
...
# Iniciar la aplicación: Cargar de la nube si existe, si no, crear desde cero
...
with st.sidebar:
    ...
    if modo_escritura:
        ...
        # --- NUEVO: SECCIÓN DE LA NUBE (POSTGRESQL) ---
        ...
        if st.button("🚀 Guardar Todo en la Nube", type="primary", use_container_width=True):
            try:
                with st.spinner("Subiendo datos a PostgreSQL..."):
                    # Preparamos la matriz para guardar (asegurando que la columna de fechas se llame 'Fecha')
                    df_mat = st.session_state.matrix_df.copy()
                    df_mat.index.name = 'Fecha'
                    
                    # Guardamos ambas tablas en la base de datos
...
with tab1:
    ...
    # --- SECCIÓN: MODIFICAR / SUSPENDER QUIRÓFANOS ---
    if modo_escritura:
        st.divider()
        ...
        if st.session_state.quirofanos_df.empty:
            ...
            if q_sel != "(Ninguno)":
                ...
                with col_elim:
                    if st.button("🗑️ Suspender / Eliminar Quirófano", type="primary", use_container_width=True):
