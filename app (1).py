...
def process_ausencias_ods(uploaded_file, matrix_df):
    ...
    if df_a.shape[1] < 4: raise ValueError("Menos de 4 columnas")
    ...
    for fila_num, row in df_a.iterrows():
        ...
        ini_raw, fin_raw, mot_raw = row['Inicio'], row['Fin'], row['Motivo']
        if med_raw in ["NAN", "", "NONE", "MEDICO"] and pd.isna(ini_raw): continue
        inicio_str, fin_str = parsear_fecha_robusta(ini_raw), parsear_fecha_robusta(fin_raw)
        motivo = str(mot_raw).strip().upper() if not pd.isna(mot_raw) else "VAC"
        if not inicio_str or not fin_str or med_raw not in staff_map: continue
...
if 'matrix_df' not in st.session_state:
    ...
    st.session_state.current_year, st.session_state.current_month = now.year, now.month
    try:
...
with st.sidebar:
    ...
    if modo_escritura:
        ...
        uploaded_guardias = st.file_uploader("1. Guardias", type=["ods", "xlsx", "csv"], key="up_g")
        if uploaded_guardias is not None and st.button("🚨 Importar Guardias", type="primary"):
            m_act, count, _ = process_guardias_ods(uploaded_guardias, st.session_state.matrix_df)
            st.session_state.matrix_df = m_act
            st.session_state.update_counter += 1
            guardar_en_nube()
            st.success(f"✅ ¡Guardias sincronizadas! ({count})")
        uploaded_consultas = st.file_uploader("2. Consultas", type=["ods", "xlsx", "csv"], key="up_c")
        if uploaded_consultas is not None and st.button("🩺 Importar Consultas", type="primary"):
            m_act, count, _ = process_consultas_ods(uploaded_consultas, st.session_state.matrix_df)
            st.session_state.matrix_df = m_act
            st.session_state.update_counter += 1
            guardar_en_nube()
            st.success(f"✅ ¡Consultas sincronizadas! ({count})")
        uploaded_ausencias = st.file_uploader("3. Ausencias", type=["ods", "xlsx", "csv"], key="up_a")
        if uploaded_ausencias is not None and st.button("🌴 Importar Ausencias", type="primary"):
            m_act, count, _ = process_ausencias_ods(uploaded_ausencias, st.session_state.matrix_df)
            st.session_state.matrix_df = m_act
            st.session_state.update_counter += 1
            guardar_en_nube()
            st.success("✅ ¡Ausencias sincronizadas!")
...
with tab1:
    ...
    if not modo_escritura: st.warning("⚠️ Modo Solo Lectura")
    ...
    if modo_escritura and st.button("Asignar Equipo al Quirófano", type="primary", use_container_width=True):
        ...
        if not equipo_nombres: st.warning("Selecciona al menos un miembro.")
        else:
            nueva_asig = pd.DataFrame([{"Fecha": q_date, "Unidad": q_unidad, "Grupo": q_grupo, "Quirófano": q_sala, "Turno": q_turno, "HC": q_hc, "Equipo": ", ".join(equipo_nombres)}])
            st.session_state.quirofanos_df = pd.concat([st.session_state.quirofanos_df, nueva_asig], ignore_index=True)
...
with tab2:
    ...
    df_adj, df_res = st.session_state.matrix_df[SURGEONS], st.session_state.matrix_df[RESIDENTS]
    st.subheader("👨‍⚕️ Adjuntos")
    if modo_escritura:
        edited_adj = st.data_editor(df_adj.style.apply(lambda x: style_matrix(df_adj), axis=None), column_config={col: st.column_config.SelectboxColumn(options=ALL_STATUSES, required=False) for col in SURGEONS}, use_container_width=True, height=600, key=f"ed_adj_{st.session_state.update_counter}")
    else:
    ...
    if modo_escritura:
        edited_res = st.data_editor(df_res.style.apply(lambda x: style_matrix(df_res), axis=None), column_config={col: st.column_config.SelectboxColumn(options=ALL_STATUSES, required=False) for col in RESIDENTS}, use_container_width=True, height=600, key=f"ed_res_{st.session_state.update_counter}")
        ...
        st.dataframe(df_res.style.apply(lambda x: style_matrix(df_res), axis=None), use_container_width=True, height=600)
...
with tab3:
    ...
    if not q_df.empty: st.dataframe(q_df.sort_values(by=["Fecha", "Unidad", "Grupo", "Turno", "Quirófano"]), hide_index=True, use_container_width=True)
